...
function dealSameRepo(index) {
	let redirect = REDIRECTS[index];
	console.log('redirect', redirect);
	const sameRepoList = REDIRECTS
		.map((r, index) => ({ index, repo: r.repo })) // 映射每个元素为包含其索引和disabled状态的对象
		.filter((r) => r.repo === redirect.repo)
	
	sameRepoList.forEach((r) => {
		toggleDisabled(r.index)
	})
